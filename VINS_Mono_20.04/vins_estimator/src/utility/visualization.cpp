...
void registerPub(ros::NodeHandle &n)
{
    ...
    pub_latest_pose = n.advertise<geometry_msgs::PoseStamped>("imu_propagate_pose", 1000);
...
void pubLatestPose(const Eigen::Vector3d &P, const Eigen::Quaterniond &Q, const Eigen::Vector3d &V, const std_msgs::Header &header)
{
    Eigen::Quaterniond quadrotor_Q = Q ;
    ...
    pose_stamped.pose.orientation.x = quadrotor_Q.x();
    pose_stamped.pose.orientation.y = quadrotor_Q.y();
    pose_stamped.pose.orientation.z = quadrotor_Q.z();
    pose_stamped.pose.orientation.w = quadrotor_Q.w();
    pub_latest_pose.publish(pose_stamped);
}
...
void pubCameraPose(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        pose_stamped.header.frame_id = "world";
        pose_stamped.pose = odometry.pose.pose;
        pub_camera_pose.publish(pose_stamped); //以geometry_msgs::PoseStamped格式发布的pose
